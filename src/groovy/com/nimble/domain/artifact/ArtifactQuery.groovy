...
class ArtifactQuery {
	...
	public static ArrayList getArtifactCategoriesForMenuTree(){
        ...
        String rootQuery = "select new map(a.id,a.display,a.display_code,'" + Artifact.TREE_ROOT + "','','') from ArtifactReference a where a.display_code not in ('PRODUCTBACKLOG', 'TASK', 'BUG', 'ISSUE', 'RISK') order by a.display"
	...
	public static ArrayList getTopLevelArtifactsForMenuTree(Long project_id, String artifactType){
    	...
    	artifactTypeQuery = "select distinct new Map(art.id,art.artifact_name,art.class,'" + Artifact.TREE_PARENT + "',art.id,art.artifact_nbr) from Artifact as art left join art.artifactChildren as artifactTree where art.project.id = " + project_id + " and art.class = '" + nh.getClassName(artifactType) + "' and artifactTree.id is null and art.component_type_ref.display_code = '- SELECT -' order by art.artifact_name"
	...
	public static ArrayList getArtifactChildrenForMenuTree(Long project_id, Long currentArtifactId, String artifactType){
       	...
       	String artifactTypeQuery = "select distinct new Map(art.id,art.artifact_name,art.class,'" + Artifact.TREE_CHILD + "',art.id,art.artifact_nbr) from Artifact as art left join art.artifactChildren as artifactTree where art.project.id = " + project_id + " and art.class = '" + nh.getClassName(artifactType) + "' and artifactTree.parent.id = " + currentArtifactId + " and art.component_type_ref.display_code = '- SELECT -' and artifactTree.relation_ref.display_code = 'RELATED' order by art.artifact_name"
