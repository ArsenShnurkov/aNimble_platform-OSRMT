...
class ReferenceController {
    ...
    def save = {
        ...
        if (referenceGroup != null) {
			...
			def mapping = GrailsDomainBinder.getMapping(referenceInstance.class)
   			ReferenceGroup refGroup = ReferenceGroup.find('from ReferenceGroup as rg where rg.reference_group=:refGroup',[refGroup:mapping.discriminator]);
   
   			if (refGroup.modification_ref.display_code.equals(ReferenceModificationReference.NOACCESS)){
   				referenceInstance.errors.reject('default.invalid.noaccess', [nh.getClassName(referenceInstance), com.nimble.domain.reference.Reference].toArray() as Object[], '')
	    	}  else if (refGroup.modification_ref.display_code.equals(ReferenceModificationReference.DISPLAY)){
   				referenceInstance.errors.reject('default.invalid.display', [nh.getClassName(referenceInstance), com.nimble.domain.reference.Reference].toArray() as Object[], '')
   			}
    ...
    def update = {
        ...
        if (referenceInstance) {
            ...
            def oldDisplayCode = referenceInstance.display_code
			...
			def mapping = GrailsDomainBinder.getMapping(referenceInstance.class)
   			...
   			if (refGroup.modification_ref.display_code.equals(ReferenceModificationReference.NOACCESS)){
        		...
        		if (!oldDisplayCode.equals(referenceInstance.display_code)){
   					referenceInstance.errors.reject('default.invalid.display', [nh.getClassName(referenceInstance), com.nimble.domain.reference.Reference].toArray() as Object[], '')
   				}
   			}
