...
class ApplicationControlController {
    ...
    def loadCache = { servletContext ->
     	...
     	def appControls = ApplicationControl.findAll("from ApplicationControl as ac where ac.visible_ind=:visible_ind order by ac.control_ref.display, ac.display_sequence, ac.display_column",[visible_ind:true])
		//println 'appControls = ' + appControls
     	...
     	//load all active view controls into app cache for retrieval by domain     	     	
     	servletContext["applicationControlsByDomain"] = viewHelper.prepViewControlsByDomain()
