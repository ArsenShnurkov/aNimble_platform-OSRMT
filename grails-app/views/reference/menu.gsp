...
%>  		
		...
		<div dojoType="dojo.data.ItemFileWriteStore"
		     url="${grailsApplication.metadata['app.namespace']}/reference/menu_tree/${params.id}?${System.currentTimeMillis()}" jsid="popStore"></div>
		...
		<script type="dojo/method" event="onClick" args="item">
		    ...
		    //dojo.byId("project_menu").innerHTML = "Loading Artifact for <a target='_blank' href='${grailsApplication.metadata['app.namespace']}/reference/show/"+  popStore.getIdentity(item) + "?reference_group=" + popStore.getValue(item, "type") + "'>link</a>";
    		if (popStore.getValue(item, "isRoot") == 'false'){
					var ref_id = popStore.getIdentity(item);
					ref_id = ref_id.substring(1,ref_id.length);
	    			dijit.byId("ProjectView").setHref("${grailsApplication.metadata['app.namespace']}/reference/show/"+  ref_id + "?reference_group=" + popStore.getValue(item, "type"));
    		} else {
    				dijit.byId("ProjectView").setHref("${grailsApplication.metadata['app.namespace']}/reference/landing?reference_group=" + popStore.getValue(item, "type"));
    		}
	        ...
	        if (popStore.getValue(item, "isRoot") == 'true'){
				...
				var fetchURL = "${grailsApplication.metadata['app.namespace']}/reference/menu_tree/${params.id}?${System.currentTimeMillis()}&reference_group=" + popStore.getValue(item, "type");
