...
class ViewHelper extends NimbleHelper{
  ...
  def getArtifactControllerName(String className){
   ...
   def getAppControlsByDomain(String domain){
		ApplicationViewControlVO appControls = servletContext.applicationControlsByDomain.get(domain)
		return appControls
   }
   ...
   def prepViewControlsByDomain(){
        HashMap viewControls = new HashMap()
	    ApplicationViewControlVO viewControl = new ApplicationViewControlVO()
	    
		for ( i in appControls ) {
		  	Hibernate.initialize(i.control_type_ref)
		  if (viewControl.domain == null){
		  	viewControl.domain = i.control_ref.display
		  	viewControl.sequenceMax = i.display_sequence
		  	viewControl.columnMax = i.display_column
		  	viewControl.fieldControls.put(i.display_sequence + '_' + i.display_column,i)
		  } else {
			if (viewControl.domain.equals(i.control_ref.display)){
			  	viewControl.fieldControls.put(i.display_sequence + '_' + i.display_column,i)

			  	if (i.display_sequence > viewControl.sequenceMax){
			  		viewControl.sequenceMax = i.display_sequence
			  	}
			  	if (i.display_column > viewControl.columnMax){
		  			viewControl.columnMax = i.display_column
		  		}
			} else {

				viewControls.put(viewControl.domain, viewControl)

			    viewControl = new ApplicationViewControlVO()
				viewControl.domain = i.control_ref.display
			  	viewControl.sequenceMax = i.display_sequence
			  	viewControl.columnMax = i.display_column
